...
const FriendWatchlist: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const friendData = friendsList.find((friend) =>
          friend.userId === parseInt(friendId as string)
        );
        ...
        if (err instanceof Error && "status" in err) {
          ...
          if (status === 401) {
            ...
            setError(
              "You don't have permission to view this friend's watchlist.",
            );
          } else if (status === 404) {
  ...
  const isInWatchlist = (movie: Movie) => {
    return currentUser?.watchlist.some((m) => m.movieId === movie.movieId) ||
      false;
  };

  const isInSeenList = (movie: Movie) => {
    return currentUser?.watchedMovies.some((m) =>
    ...
    try {
      ...
      if (error instanceof Error && "status" in error) {
        ...
        switch (appErr.status) {
          case 401:
            setActionError(
              "Please log in again to add movies to your watchlist.",
            );
            break;
          case 403:
            setActionError(
          ...
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-3xl p-6 mb-8 shadow-lg text-white">
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {/* Favorite Directors */}
                {friend.favoriteDirectors &&
                  friend.favoriteDirectors.length > 0 && (
                  <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 hover:bg-white/20 transition-all">
                    <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                      <svg
                        className="w-4 h-4"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-white/20">
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold">
                    {(friend.favoriteActors?.length || 0) +
                      (friend.favoriteDirectors?.length || 0)}
                  </p>
