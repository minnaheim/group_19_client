...
const GroupsManagement: React.FC = () => {
    ...
    return (
        <div className="bg-[#ebefff] flex flex-col md:flex-row min-h-screen w-full">
            {/* Sidebar navigation */}
            <Navigation userId={userId} activeItem="Your Groups" />

            {/* Main content */}
            <div className="flex-1 p-4 md:p-6 lg:p-8 overflow-auto">
                <div className="mb-8">
                    <h1 className="font-semibold text-[#3b3e88] text-3xl">Your Movie Groups</h1>
                    ...
                    <>
                        ...
                        {displayGroups.length > 0 ? (
                                    ...
                                    >
                                        <div>
                                            <div className="flex items-center justify-between mb-2">
                                                <h3 className="font-semibold text-[#3b3e88] text-lg">{group.name}</h3>
                          ...
                          {group.members.length} {group.members.length === 1 ? 'member' : 'members'}
                                            ...
                                            {group.description && (
                                                <p className="text-[#838bad] text-sm mb-3">{group.description}</p>
                                            )}

                                            {/* Movie pool preview */}
                                            {group.moviePool && group.moviePool.length > 0 ? (
                                                <div className="mb-4">
                                                    <p className="text-xs text-[#3b3e88]/60 mb-2">Movie Pool ({group.moviePool.length})</p>
                                                    <div className="flex gap-2 overflow-x-auto pb-2">
                                                        {group.moviePool.slice(0, 4).map((movie) => (
                                                            <div key={movie.movieId} className="w-10 h-14 flex-shrink-0 rounded overflow-hidden">
                                                                <img
                                                                    src={movie.posterURL}
                                                                    alt={movie.title}
                                                        ...
                                                        {group.moviePool.length > 4 && (
                                                            <div className="w-10 h-14 bg-indigo-100 flex-shrink-0 flex items-center justify-center rounded">
                                                                <span className="text-xs font-medium text-indigo-700">+{group.moviePool.length - 4}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                            ) : (
                                                <p className="text-[#b9c0de] text-xs mb-4">No movies in pool yet</p>
                                            )}

                                            <div className="flex justify-between items-center text-xs text-[#b9c0de]">
                                                <span>Created {new Date(group.createdAt).toLocaleDateString()}</span>
                                                <span>{group.creator.username === "current_user" ? "Created by you" : `Created by ${group.creator.username}`}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="bg-white rounded-3xl p-12 text-center shadow-sm">
                                {searchQuery ? (
                                    <div>
                                        <p className="text-[#838bad] mb-4">No groups match your search &#34;{searchQuery}&#34;</p>
                                        <Button
                                            variant="outline"
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[#838bad] mb-2">You aren&#39;t part of any groups yet</p>
                                        <p className="text-[#b9c0de] mb-6">Create a group to start watching movies with friends!</p>
                                        <Button
                ...
                {activeTab === 'invitations' && (
                    <div className="space-y-6">
                        ...
                        {receivedInvitations.length > 0 && (
                            <div className="mb-8">
                                <h3 className="text-[#3b3e88] font-medium text-lg mb-4">Received Invitations</h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {receivedInvitations.map(invitation => (
                                        <div key={invitation.id} className="bg-white rounded-2xl p-4 shadow-sm border-l-4 border-rose-500">
                                            <div className="mb-3">
                                                <h4 className="font-semibold text-[#3b3e88]">{invitation.group.name}</h4>
                                                <p className="text-[#b9c0de] text-xs mb-1">
                                                    Invited by {invitation.sender.username} on {new Date(invitation.createdAt).toLocaleDateString()}
                                                </p>
                                                {invitation.group.description && (
                                                    <p className="text-[#838bad] text-xs italic mb-2">&#34;{invitation.group.description}&#34;</p>
                                                )}
                                            </div>
                        ...
                        {sentInvitations.length > 0 && (
                            <div>
                                <h3 className="text-[#3b3e88] font-medium text-lg mb-4">Sent Invitations</h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {sentInvitations.map(invitation => (
                                        <div key={invitation.id} className="bg-white rounded-2xl p-4 shadow-sm border-l-4 border-orange-400">
                                            <div className="mb-3">
                                                <h4 className="font-semibold text-[#3b3e88]">{invitation.group.name}</h4>
                                                <p className="text-[#b9c0de] text-xs mb-1">
                                                    Invited {invitation.receiver.username} on {new Date(invitation.createdAt).toLocaleDateString()}
                                                </p>
                                                <p className="text-[#838bad] text-xs italic mb-1">Status: Pending</p>
                                            </div>
                                            <Button
                                                variant="outline"
                        ...
                        {receivedInvitations.length === 0 && sentInvitations.length === 0 && (
                            <div className="bg-white rounded-3xl p-12 text-center shadow-sm">
                                ...
                                <p className="text-[#b9c0de]">Invite friends to your groups or wait for invitations</p>
                            </div>
                        )}
                    </div>
                )}
                ...
                <Dialog open={isCreateGroupDialogOpen} onOpenChange={setIsCreateGroupDialogOpen}>
                    <DialogContent className="max-w-md rounded-2xl">
                        <DialogHeader>
                            <DialogTitle className="text-[#3b3e88] text-xl">Create New Group</DialogTitle>
                        </DialogHeader>
                        <form onSubmit={handleCreateGroup}>
                            <div className="space-y-4 py-4">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="group-description" className="text-[#3b3e88]">Description (Optional)</Label>
                                    <Input
                ...
                <Dialog open={isInviteDialogOpen} onOpenChange={setIsInviteDialogOpen}>
                    <DialogContent className="max-w-md rounded-2xl">
                        <DialogHeader>
                            <DialogTitle className="text-[#3b3e88] text-xl">Invite to Group</DialogTitle>
                        </DialogHeader>
                        <form onSubmit={handleInviteMember}>
                            <div className="space-y-4 py-4">
                                <div className="space-y-2">
                                    <Label htmlFor="invite-username" className="text-[#3b3e88]">Friend&#39;s Username</Label>
                                    <Input
                                        id="invite-username"
                ...
                <Dialog open={isGroupDetailDialogOpen} onOpenChange={setIsGroupDetailDialogOpen}>
                    <DialogContent className="max-w-3xl rounded-2xl">
                        {selectedGroup && (
                            <>
                                <DialogHeader>
                                    <DialogTitle className="text-[#3b3e88] text-xl">{selectedGroup.name}</DialogTitle>
                                </DialogHeader>
                                <div className="py-4">
                                    {selectedGroup.description && (
                                        <p className="text-[#838bad] mb-4">{selectedGroup.description}</p>
                                    )}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <div className="bg-indigo-50 rounded-xl p-4 space-y-2">
                                                <div>
                                                    <span className="text-[#838bad] text-sm">Created by:</span>
                                                    <p className="text-[#3b3e88] font-medium">
                                                        {selectedGroup.creator.username} {selectedGroup.creator.userId === parseInt(id as string) && "(You)"}
                                                    </p>
                                                </div>
                                                <div>
                                                    <span className="text-[#838bad] text-sm">Created on:</span>
                                                    <p className="text-[#3b3e88]">{new Date(selectedGroup.createdAt).toLocaleDateString()}</p>
                                                </div>
                                                <div>
                                                    <span className="text-[#838bad] text-sm">Members ({selectedGroup.members.length}):</span>
                                                    <p className="text-[#3b3e88]">{formatMembersList(selectedGroup.members)}</p>
                                                </div>
                                            </div>

                                            <div className="mt-6">
                                                <h4 className="font-medium text-[#3b3e88] mb-2">Actions</h4>
                                                <div className="space-y-2">
                                                    <Button
                                                        onClick={() => {
                                                    ...
                                                    {selectedGroup.creator.userId !== parseInt(id as string) && (
                                                        <Button
                                        ...
                                        <div>
                                            ...
                                            {selectedGroup.moviePool && selectedGroup.moviePool.length > 0 ? (
                                                    ...
                                                    {selectedGroup.moviePool.map((movie) => (
                                                        <div key={movie.movieId} className="flex items-center gap-3 bg-white p-2 rounded-xl shadow-sm">
                                                            ...
                                                            <div className="flex-grow min-w-0">
                                                                <p className="font-medium text-[#3b3e88] truncate">{movie.title}</p>
                                                                <p className="text-xs text-[#b9c0de]">{movie.year} • {movie.genres.slice(0, 2).join(", ")}</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                                ...
                                                <div className="bg-indigo-50 rounded-xl p-6 text-center">
                                                    <p className="text-[#838bad] mb-2">No movies in pool yet</p>
                                                    <p className="text-[#b9c0de] text-sm mb-4">Add movies to the pool for your group to vote on</p>
                                                </div>
                                            )}

                                            <div className="grid grid-cols-2 gap-3 mt-4">
                                                <Button
                                                    onClick={() => navigateToGroupPool(selectedGroup.groupId)}
                                                    className="bg-[#3b3e88] hover:bg-[#3b3e88]/90 rounded-xl text-sm"
                                                >
                                                    Manage Pool
                                                </Button>
                                                <Button
                                                    onClick={() => navigateToGroupVoting(selectedGroup.groupId)}
                                                    className="bg-indigo-600 hover:bg-indigo-700 rounded-xl text-sm"
                                                >
                                                    Vote
                                                </Button>
                                                <Button
                                                    onClick={() => navigateToGroupResults(selectedGroup.groupId)}
